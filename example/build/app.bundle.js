...
var SubComponent = __webpack_require__(25);
...
var App = function (_React$Component) {
    ...
    function App(props) {
        ...
        _this.state = { items: [{ value: 100 }], text: 'hello!' };
    ...
    _createClass(App, [{
        ...
        value: function updateList() {
            ...
            this.setState({ items: items, text: 'text ' + Math.random() });
        }
    }, {
        key: 'render',
...
var React = __webpack_require__(1);
var PolymerComponent = __webpack_require__(26);
var MyPolymerElement = PolymerComponent.bind('my-polymer-element');
                ...
                React.createElement(
                    'my-polymer-element',
                    {
                        ...
                        'on-click': function onClick(e) {
                            return console.log('Polymer Click Event');
                        },
                        'on-my-event': function onMyEvent(e) {
                            return console.log('Custom Polymer Event');
                        }
                    },
                    React.createElement(
                        'div',
...
var PolymerComponent = function (_React$Component) {
        _inherits(PolymerComponent, _React$Component);
        ...
        _createClass(PolymerComponent, [{
                ...
                value: function bind(tag) {

                        return function (_PolymerComponent) {
                                _inherits(_class, _PolymerComponent);
        ...
        function PolymerComponent(props) {
                _classCallCheck(this, PolymerComponent);

                // the events that have been requested to be
                // registered to
                var _this = _possibleConstructorReturn(this, (PolymerComponent.__proto__ || Object.getPrototypeOf(PolymerComponent)).call(this, props));

                _this.events = null;

                // the original values of all the properties for the
                // element
                _this.originalProps = null;
        ...
        _createClass(PolymerComponent, [{
                ...
                value: function shouldComponentUpdate(newProps) {
                        ...
                        var tag = newProps['element-tag'] || this.tag;
                        var hasNewTag = tag.toLowerCase() !== this.tag.toLowerCase();
                ...
                key: '_updateDefaults',
                value: function _updateDefaults() {

                        this.events = {};

                        var cl = customElements.get(this.tag);
                        var properties = cl ? cl.properties : null;
                        this.originalProps = {};
                        for (var key in properties) {
                                this.originalProps[key] = this.element[key];
                        }
                }
        }, {
                key: '_updateElementProperties',
                value: function _updateElementProperties(props) {
                        var _this3 = this;

                        var removeEvent = function removeEvent(key) {

                                _this3.element.removeEventListener(key.replace(/^on-/, ''), _this3.events[key]);
                                delete _this3.events[key];
                        };

                        var el = this.element;
                        var newProps = Object.assign({}, this.originalProps, props);
                        ...
                        var _loop = function _loop(key) {
                                ...
                                if (key in _this3.originalProps && el.get(key) !== val) el.set(key, val);

                                // register events based on the "on-" attribute keywords
                                if (/^on-/.test(key)) {

                                        // remove the event if it's different
                                        if (key in _this3.events && _this3.events[key] !== val) removeEvent(key);

                                        if (!(key in _this3.events)) {

                                                _this3.events[key] = function (e) {
                                                        return props[key](e);
                                                };
                                                _this3.element.addEventListener(key.replace(/^on-/, ''), _this3.events[key]);
                                        }
                                }
                        };
                        ...
                        // styles
                        var style = '';
                        for (var _key in newProps.style) {

                                style += _key + ':' + newProps.style[_key] + ';';
                        }
